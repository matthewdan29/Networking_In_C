...
/*Our HTTP server, like all servers, needs to create a listening socket to accept new connections. 
 * Next, is the function "create_socket()" for this purpose.*/
...
/*It is useful to define a C "struct" to store information on each connected client*/
...
/*"get_client()" takes a "SOCKET" as its input and return a "client_info" structure.*/
...
/*"get_client_address()" well it gets the clients address */
...
/*"wait_on_clients()" blocks until an exiting clinet sends data or a new client attemps to connect.*/
...
/*"send_400" sends a bad request or for any of the several situaltions that might happen*/
...
/*"send_404" is for sending a bad request message back to the client*/
...
/*"serve_resource()" function takes as arguments as connected client and a requested resource path. */
