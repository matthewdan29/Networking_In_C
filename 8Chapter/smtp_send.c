...
/*Below is two constants to make buffer allocation and checking easier*/
#define MAXINPUT 512
...
/*"send_format()" function sends formatted strings directly over the network*/
...
/*"parse_response()" makes sure single multiline response are reconized*/
...
/*"wait_on_response()" waits until a particular response code is received over the network.*/
...
/*"connect_to_host()" attempts to open a TCP connection to a given hostname and port number. */
...
int main()
{
	...
	wait_on_response(*sender, 250); /////////here
